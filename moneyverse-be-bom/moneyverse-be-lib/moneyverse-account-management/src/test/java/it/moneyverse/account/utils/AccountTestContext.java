...
public class AccountTestContext extends TestContext {
    ...
    public AccountRequestDto createAccountForUser(String username) {
        return toAccountRequest(
                MapperTestHelper.map(
                        new FakeAccount(username, model.getAccounts().size()), Account.class));
    }
    ...
    public static AccountRequestDto createAccountWithNullUsername(String username) {
        Account account =
                MapperTestHelper.map(
                        new FakeAccount(username, getCurrentInstance().getModel().getAccounts().size()), Account.class);
    ...
    public List<AccountModel> filterAccounts(AccountCriteria criteria) {
        return model.getAccounts().stream()
                .filter(account -> criteria.getUsername()
                        .map(username -> username.equals(account.getUsername()))
                        .orElse(true))
                .filter(account -> criteria.getBalance()
                        .map(balanceCriteria -> account.getBalance() != null && filterByBound(account.getBalance(), balanceCriteria))
                        .orElse(true))
                .filter(account -> criteria.getBalanceTarget()
                        .map(balanceTargetCriteria -> account.getBalanceTarget() != null && filterByBound(account.getBalanceTarget(), balanceTargetCriteria))
                        .orElse(true))
                .filter(account -> criteria.getAccountCategory()
                        .map(category -> category.equals(account.getAccountCategory()))
                        .orElse(true))
                .filter(account -> criteria.getDefault()
    ...
    private boolean filterByBound(BigDecimal value, BoundCriteria boundCriteria) {
        return boundCriteria.getLower()
                .map(lower -> value.compareTo(lower) >= 0)
                .orElse(true) &&
                boundCriteria.getUpper()
                        .map(upper -> value.compareTo(upper) <= 0)
                        .orElse(true);
    }

    private int sortByCriteria(AccountModel a, AccountModel b, SortCriteria<AccountSortAttributeEnum> sortCriteria) {
        ...
        int comparison = switch (attribute) {
